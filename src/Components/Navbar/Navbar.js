...
export default function Navbar(props) {
  return (
    props.setHome && <div className={`nav bg-black flex justify-evenly ${props.isBurger?'h-[200px]':'h-[70px]'}  w-[100%] sticky z-[100] space-x-40 xl:h-[70px]`}>
      ...
      <button className='burger absolute top-5 right-10 w-[20px] h-[15px] xl:hidden' onClick={props.clicking}>
      ...
      <div className={`resnavitem ${props.isBurger?'block':'hidden'} w-[100%] text-white xl:flex xl:justify-around`}>
        ...
        <div className='px-3 rounded-sm home ml-3 flex space-x-2'>
          ...
          <button className='pr-2 mt-4 xl:mb-6' onClick={props.onCat}>Categories</button>
        </div>
      </div>
